...
class Pag():
    ...
    async def start(self, ctx):
        ...
        (self.pages[0]).set_footer(text=f'Page 1 out of {len(self.pages)}\nIf any results seem like they are missing check the sheet in a broswer')
        ...
        while not timedout:
            try:
                ...
                if self.currentpage != previous_page:
                    (self.pages[self.currentpage]).set_footer(text=f'Page {self.currentpage+1} out of {len(self.pages)}\nIf any results seem like they are missing check the sheet in a broswer')
    ...
    async def chunktopage(self, chunk: list, color: discord.Color, title: str):
